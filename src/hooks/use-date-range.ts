...
export function useDateRange({ selected, onSelect }: UseDateRangeProps) {
  ...
  function formatDateRange() {
    if (!selected.startDate || !selected.endDate) return 'Selecione o período'

    const startFormatted = selected.startDate.format('MMM DD, YYYY')
    const endFormatted = selected.endDate.format('MMM DD, YYYY')
